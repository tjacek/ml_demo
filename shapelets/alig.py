...
def shapelet(x,y,start=40,step=80):
...
def shapelet_plot(ts,s):
    ...
    plt.ylabel("Wartość szeregu czasowego",fontsize=15) 
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def distance_plot(ts,s):
    ...
    plt.xlabel("Początek podciągu",fontsize=15) 
    plt.ylabel("Odległość",fontsize=15) 
    plt.tight_layout()
